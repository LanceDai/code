...
int bitc[1 << MAXN];
int p2[100000];
int g[1 << MAXN][MAXN + 1], f[1 << MAXN][MAXN + 1];

void fwt(int f[1 << MAXN][MAXN + 1]) {
...
int main() {
    ...
    for (int S = (int) 0; S <= (int) (1 << n) - 1; S++) {
        int w = 0;
        for (int i = (int) 0; i <= (int) n - 1; i++)if (S & (1 << i))w += bitc[(query[i] & S)];
        num[S] = w / 2;
    }
    p2[0] = 1;
